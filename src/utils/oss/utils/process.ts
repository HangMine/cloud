...
import { OssProcess, OssProcessCommandsMap, OssProcessParamsMap } from './typings.d';
  ...
  return Object.fromEntries(moduleStrs.map((moduleStr) => {
    ...
    return [moduleName, Object.fromEntries(commandStrs.map((commandStr) => {
      ...
      return [commandName,
        paramStrs.length === 1
          ? paramStrs[0]
          : Object.fromEntries(paramStrs.map((paramStr) => {
            // 参数名与参数值之间用_隔开
...
export function parseUrl(url: string) {
  ...
  const { query: { 'x-oss-process': xOssProcessArr, ...query }, url: outputUrl } = qs.parseUrl(url);
  const xOssProcess = first(xOssProcessArr);
  if (xOssProcess) {
...
export function stringify(process: OssProcess) {
  return Object.entries(process)
    .filter((entry): entry is [string, string | number | OssProcessCommandsMap] => {
      return !!entry[1];
    })
    .map(([moduleName, commandsMap]) => {
      ...
      return `${moduleName}/${Object.entries(commandsMap)
        .filter((entry): entry is [string, string | number | OssProcessParamsMap] => {
          return !!entry[1];
        })
        .map(([commandName, paramsMap]) => {
          ...
          return `${commandName},${typeof paramsMap === 'string'
            ? paramsMap
            : Object.entries(paramsMap).map(([paramName, paramValue]) => {
...
export function stringifyUrl({ url, process }: { url: string, process: OssProcess }) {
  const parsed = parseUrl(url);
  console.log(parsed, merge(parsed.process, process));
