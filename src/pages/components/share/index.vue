...
import {
  defineComponent, reactive, computed, ref, toRefs, markRaw, PropType, onUnmounted, nextTick,
} from 'vue';
  ...
  setup(props, { emit }) {
    ...
    const copyLink = async () => {
      try {
        ...
        const copyStr = `${window.location.host}/main/share?shareId=${shareId}${code ? `\n访问密码：${code}` : ''}`;
        copy(copyStr);
        ElMessage.success('复制成功');
      } finally {
